...
def _execute(filters, additional_table_columns=None):
	...
	for inv in invoice_list:
		...
		location = list(set(invoice_so_dn_map.get(inv.name, {}).get("custom_location", [])))
		po_no= list(set(invoice_so_dn_map.get(inv.name, {}).get("po_no", [])))
	...
	res=get_location_based(res,filters)
...
# 			)
#     frappe.errprint(invoice_so_dn_map)
...
def get_invoice_so_dn_map(invoice_list):
    ...
    query = f"""
        ...
        AND (sii.sales_order != '' OR sii.delivery_note != '')
    """
...
def get_location_based(invoice_list, filters=None):
    if not filters:
        return invoice_list
    ...
    return [
        ...
        if (location is None or inv.get("location") == location) and
           (po_no is None or inv.get("po_no") == po_no)
    ]
